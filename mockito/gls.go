...
func recordLast(s *state, codePtr uintptr, args []reflect.Value, numOut int, matchers []Matcher) {
	glsMu.Lock()
	lasts[gls.Key()] = &pendingCall{state: s, codePtr: codePtr, args: snapshot(args), numOut: numOut, matchers: matchers}
	glsMu.Unlock()
}
