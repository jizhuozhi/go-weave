...
// that move pointers through the stack no longer need compile-time codegen
// (StubSource / go generate). The trampoline itself is the same register
// shuffle the generated stubs perform; only the moduledata is shaped per
// method, and the argument pointer map is built directly rather than derived
// from a compiled function signature.
//
...
func jitTrampoline(sh stubShape) unsafe.Pointer {
	code := jitStubCode(sh.index, uintptr(reflect.ValueOf(Dispatch).Pointer()))
	...
	jitMu.Lock()
	md := buildJITModule(mem[:len(code)], sh.argWords, sh.retWords, sh.argPtrs, sh.retPtrs)
	registerModule(md)
