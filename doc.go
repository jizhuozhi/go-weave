...
// # Runtime JIT
//
// On Go 1.23+ the precise trampoline is generated at runtime rather than by the
// compiler: the proxy mmaps an executable page, writes the trampoline's machine
// code (the same register shuffle the generic stubs perform), and builds a
// moduledata whose argument pointer map describes the stack area word by word.
// The module is appended to runtime.lastmoduledatap via linkname, so findfunc
// recognises the injected code and the collector scans any pointer in the stack
// area exactly as it would for a compiled function. Proxy construction therefore
// needs no go generate and no pre-declared shape. The forged moduledata layout
// is version-specific, so the JIT path is gated to Go 1.23+; earlier releases
// use StubSource.
//
...
//     a trampoline that describes that area to the collector: on Go 1.23+ one
//     is generated at runtime (see "Runtime JIT" above); earlier releases fall
//     back to StubSource. Since register assignment is positional, moving
//     pointer arguments to the front of a signature is often enough to avoid
//     the stack area altogether.
